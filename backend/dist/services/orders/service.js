...
exports.updateOrderItems = exports.getOrdersByShop = exports.getOrdersByCustomer = exports.createOrder = void 0;
exports.updateOrderStatus = updateOrderStatus;
exports.verifyOrderPrice = verifyOrderPrice;
exports.getOrderById = getOrderById;
const database_1 = require("../../shared/database");
const logger_1 = require("../../shared/utils/logger");
const ApiError_1 = require("../../shared/errors/ApiError");
const createOrder = async (shopId, customerId, items, offerCode, paymentPreference, fulfillmentMethod) => {
    // ... existing validation ...
    const shop = await database_1.prisma.shop.findUnique({ where: { shopId } });
    if (!shop)
        throw new ApiError_1.ApiError(404, 'Shop not found');
    const OfferService = await Promise.resolve().then(() => __importStar(require('../offers/service')));
    return database_1.prisma.$transaction(async (tx) => {
        ...
        const processedItems = await Promise.all(items.map(async (item) => {
            ...
            if (item.productId) {
                ...
                if (!product) {
                    throw new ApiError_1.ApiError(400, `Product not found or invalid: ${item.name}`);
                }
                if (product.stock < item.quantity) {
                    throw new ApiError_1.ApiError(400, `Insufficient stock for product: ${product.name}. Available: ${product.stock}, Requested: ${item.quantity}`);
                }
                // Update stock
            ...
            // Ad-hoc
            if (!item.name)
                throw new ApiError_1.ApiError(400, 'Items must have a name');
...
async function updateOrderStatus(orderId, status) {
    ...
    if (status === 'ACCEPTED' && !order.priceVerified) {
        // Check if this is a manual order (has items without productId)
        const items = order.items;
        const hasManualItems = items.some(item => !item.productId);
        if (hasManualItems) {
            throw new ApiError_1.ApiError(400, 'Cannot accept order. Please verify prices first.');
        }
    }
    ...
    if (status === 'COLLECTED' && order.status !== 'COLLECTED') {
        ...
        try {
            ...
            await LedgerService.recordSale(order.shopId, Number(order.totalAmount), paymentType, 'ORDER', order.customerId, `Order #${order.orderId.slice(0, 8)}`, true, // Bypass credit limit check on order fulfillment
            profit);
        }
        catch (err) {
            logger_1.logger.error(`Failed to record sale for collected order ${orderId}:`, err);
        }
    }
    return updated;
}
 ...
 * Verify order prices
 */
async function verifyOrderPrice(orderId, verifiedBy) {
    const order = await database_1.prisma.order.findUnique({ where: { orderId } });
    if (!order) {
        throw new ApiError_1.ApiError(404, 'Order not found');
    }
    ...
    if (hasInvalidPrices) {
        throw new ApiError_1.ApiError(400, 'Cannot verify order with zero or missing prices. Please update prices first.');
    }
...
const getOrdersByCustomer = async (customerId) => {
    return database_1.prisma.order.findMany({ where: { customerId }, orderBy: { createdAt: 'desc' } });
};
exports.getOrdersByCustomer = getOrdersByCustomer;
const getOrdersByShop = async (shopId) => {
    return database_1.prisma.order.findMany({ where: { shopId }, orderBy: { createdAt: 'desc' } });
};
exports.getOrdersByShop = getOrdersByShop;
...
const updateOrderItems = async (orderId, items, newStatus) => {
    // We need to fetch current order to maintain discount and delivery charge
    const currentOrder = await database_1.prisma.order.findUnique({ where: { orderId } });
    if (!currentOrder)
    ...
    if (newStatus) {
        ...
        if (newStatus === 'ACCEPTED') {
            throw new ApiError_1.ApiError(400, 'Cannot accept order directly when updating items. Please update items, then verify prices separately.');
        }
