...
class BackupService {
    async createBackup(shopId) {
        ...
        const sales = await database_1.prisma.sale.findMany({ where: { shopId } });
        const orders = await database_1.prisma.order.findMany({ where: { shopId } });
        const products = await database_1.prisma.product.findMany({ where: { shopId } });
        const customerIds = new Set();
        sales.forEach(s => { if (s.customerId)
            customerIds.add(s.customerId); });
        orders.forEach(o => { if (o.customerId)
    ...
    async restoreBackup(shopId, data) {
